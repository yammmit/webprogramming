...
export default function LadderResultModal({ visible, result, onClose, onViewTask }) {
  ...
  /* -------------------------------------------------------
     1) 그룹 멤버 API로부터 authoritative participants 가져오기
  -------------------------------------------------------- */
  useEffect(() => {
  ...
  /* -------------------------------------------------------
     3) participants enrichment fallback
  -------------------------------------------------------- */
  useEffect(() => {
    if (!visible || !result) return;
    ...
    (async () => {
      try {
        const missingIds = [];
        const base = result.participants.map((p, idx) => {
          ...
          const looksPlaceholder =
            /^\s*$/.test(rawName) ||
            /^참여자\s*\d+$/.test(rawName) ||
            /^\d+$/.test(rawName) ||
            String(rawName) === String(id);
        ...
        let fetched = [];
        if (uniq.length > 0) fetched = await fetchUsersByIds(uniq);

        const nameMap = {};
        fetched.forEach(u => {
          const myId = Number(u.user_id || u.id);
          nameMap[myId] = u.user_name || u.name || u.displayName;
        });
  ...
  // build resolvedParticipants: authoritative names from group members, enrichment, or fetched user ids
  useEffect(() => {
    if (!visible || !result) return;
    (async () => {
      try {
        // prefer localParticipants (group members)
        ...
        if (Array.isArray(result.participants) && result.participants.length > 0) {
          ...
          console.debug('LadderResultModal: initial from result.participants', { initial, ids, assignedId, winnerId });
          ...
          console.debug('LadderResultModal: fetched users for ids', { uniq, fetched });
          const idMap = {};
          fetched.forEach(u => { idMap[Number(u.user_id) || Number(u.id)] = u.user_name || u.name; });

          // also include localParticipants/enrichedParticipants in idMap to prefer authoritative names
        ...
        // else fallback to enrichedParticipants
  ...
  /* -------------------------------------------------------
     4) participants 결정 로직
     우선순위: groupMembers > result.participants > enriched > fallback
  -------------------------------------------------------- */
  // use resolvedParticipants if available so UI updates when async lookups complete
  let participants = [];
  ...
  } else if (Array.isArray(result.participants) && result.participants.length > 0) {
    participants = (enrichedParticipants ?? result.participants).map((p, idx) => ({
      user_id: p.user_id ?? p.id ?? idx + 1,
      user_name: p.user_name ?? p.name ?? `참여자 ${idx + 1}`,
      name: p.user_name ?? p.name ?? `참여자 ${idx + 1}`
    }));
  } else if (Array.isArray(enrichedParticipants) && enrichedParticipants.length > 0) {
    participants = enrichedParticipants;
  } else if (result.total_members > 0) {
    participants = Array.from({ length: result.total_members }).map((_, i) => ({
      user_id: i + 1,
      user_name: `참여자 ${i + 1}`
    }));
  } else {
    participants = [
      { user_id: 1, user_name: '참여자 1' },
      { user_id: 2, user_name: '참여자 2' }
    ];
  }
  ...
  /* -------------------------------------------------------
     5) winner index based ONLY on server bottom_result
  -------------------------------------------------------- */
  ...
  // compute displayWinnerName by mapping assigned_to/winner to resolved participant names if possible
  let displayWinnerName = null;
  const assignedIdObj = result.assigned_to && typeof result.assigned_to === 'object' ? Number(result.assigned_to.user_id ?? result.assigned_to.id) : null;
  const assignedIdRaw = result.assigned_to && typeof result.assigned_to !== 'object' ? Number(result.assigned_to) : null;
  const assignedId = !Number.isNaN(assignedIdObj) ? assignedIdObj : (!Number.isNaN(assignedIdRaw) ? assignedIdRaw : null);
  const winnerObjId = result.winner && (result.winner.user_id ?? result.winner.id) ? Number(result.winner.user_id ?? result.winner.id) : null;
  if (assignedId != null) {
    const found = participants.find(p => Number(p.user_id) === Number(assignedId));
    if (found) displayWinnerName = found.user_name ?? found.name;
  }
  if (!displayWinnerName && winnerObjId != null) {
    const found2 = participants.find(p => Number(p.user_id) === Number(winnerObjId));
    if (found2) displayWinnerName = found2.user_name ?? found2.name;
  }
  // fallback to participant at winIdx
  if (!displayWinnerName && winIdx >= 0) displayWinnerName = participants[winIdx]?.user_name ?? participants[winIdx]?.name;
  // final fallback: server-provided winner name if any
  ...
  /* -------------------------------------------------------
     6) 사다리 선 (ladder_map → connections)
  -------------------------------------------------------- */
  let ladder_map = Array.isArray(result.ladder_map) ? result.ladder_map : [];
  // if server did not provide ladder_map, generate a deterministic fallback so rows are visible
  if (!Array.isArray(ladder_map) || ladder_map.length === 0) {
    ...
    const rows = 7; // default rows for fallback
    const gen = [];
  ...
  let levels = 6;
  if (ladder_map.length > 0) {
    levels = Math.max(...ladder_map.map(r => Number(r.row || 0)));
  }
  ...
  const xPositions = Array.from({ length: columns }).map((_, i) =>
    Math.round(leftPad + (usable * i) / Math.max(columns - 1, 1))
  );
  ...
  const connections = Array.from({ length: levels }).map(() =>
  ...
  ladder_map.forEach(item => {
    ...
    if (row >= 1 && row <= levels && col >= 1 && col <= columns - 1) {
      connections[row - 1][col - 1] = true;
    }
  });

  /* -------------------------------------------------------
     7) path simulation: choose startIndex that leads to winner
     - prefer inverting result.result_map if provided
     - otherwise brute-force simulate each start to find the one that lands on winIdx
     - fallback to center
  -------------------------------------------------------- */
  function simulateEndFromStart(startIdx) {
    ...
    return cur; // 0-based end index
  }

  let startIndex;
  // try to invert server result_map (1-based mapping)
  ...
  // if not found, brute-force simulate
  if (typeof startIndex === 'undefined' && winIdx >= 0) {
    for (let s = 0; s < columns; s++) {
      try {
        if (simulateEndFromStart(s) === winIdx) { startIndex = s; break; }
      } catch (e) {}
    }
  }
      ...
      <div style={{ width: 420, background: '#fff', borderRadius: 12, padding: 18, boxShadow: '0 10px 30px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
            ...
            {/* top badges: show '당첨' at the animation start column so ladder visibly begins from winner */}
            {xPositions.map((x, i) => {
            ...
            {connections.map((row, lvl) =>
              row.map((on, i) => {
                ...
                const y = Math.round(topY + (lvl + 1) * levelStep);
                return (
                  <line key={`rung-${lvl}-${i}`} x1={xPositions[i]} y1={y} x2={xPositions[i + 1]} y2={y} stroke="#e6e6e6" strokeWidth={2} />
                );
              })
            )}
            ...
            {/* bottom labels (participant names) */}
            {xPositions.map((x, i) => (
              <text key={`bot-${i}`} x={x} y={bottomY + 32} textAnchor="middle" style={{ fontSize: 12, fontWeight: 700, fill: '#222' }}>
                {participants[i]?.user_name ?? participants[i]?.name ?? participants[i]?.user?.user_name ?? participants[i]?.user?.name ?? `참여자 ${i+1}`}
              </text>
            ))}
